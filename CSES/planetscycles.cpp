#include <bits/stdc++.h> //AAAAAAAH IT WORKED FIRST TRY NO WAY
using namespace std;
//idea:
//find all cycles in the graph with dfs. Then for each node in the cycle, work backwards with a reverse adjacency list through all non-cyclical children. 
//All nodes in a cycle will visit each node in the cycle before returning to themselves. For each node not in a cycle, each step removed from a cyclical node is one extra move.
...
void Floyd(int node){ //standard Floyd algo with some modifications
	...
	for(int a : tempc){ //setting all the answers of the nodes in the cycle to the number of nodes in the cycle
		answers[a] = ta;
	}
...
void revDFS(int node, int cs){//DFS'ing through the reverse adjacency list of the node
	answers[node] = cs; //cs is the current amount of teleports
//	cout<<"aa";
	for(int c : revAdj[node]){
		if(answers[c] == 0){ //this is why I had to assign the answers for each cyclical node in each group back in the floyd function, it was repeatedly DFSing already visited nodes
			revDFS(c, cs+1);//since each unvisited child is one more step removed from the cyclem, the answer goes up by 1	
		}
