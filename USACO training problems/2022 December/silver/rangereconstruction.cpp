	...
	for(int c = 2; c < n; c++){
		...
		if(p1 == p2){
			...
			for(int b = c-1; b >=0; b--){
				...
				if(*max_element(s1,e1) - *min_element(s1,e1)  == lists[b][n-b-1] && *max_element(s2,e2) - *min_element(s2,e2)  == lists[b][n-b-1]){
				
					continue;
				
				}
				else if(*max_element(s1,e1) - *min_element(s1,e1)  == lists[b][n-b-1] && *max_element(s2,e2) -*min_element(s2,e2) != lists[b][n-b-1]){
